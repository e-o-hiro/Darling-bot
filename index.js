...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'Hiro ModZ\n'
            + 'DARLING BOT;\n' // Nome do bot
            + 'TEL;type=CELL;type=VOICE;waid=557996004602:+55 79 9600-4602\n' //Seu número Whatsapp
            + 'END:VCARD'
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				success: '〔🆗〕〔Concluido!!〕',
				error: {
					stick: '〔❗〕〔correu um erro ao converter a imagem em um adesivo〕',
					Iv: '〔❗〕〔Link inválido〕'
				},
				only: {
			...
			const ownerNumber = [`557996004602@s.whatsapp.net`] // substitua isso pelo seu número
            const mod = [ownerNumber,"557996004602@s.whatsapp.net"]//mude o seu numero
            const adminbotnumber = ["557996004602@s.whatsapp.net"]//mude o seu numero
			const frendsowner = ["557996004602@s.whatsapp.net"]//mude o seu numero
...
case 'antifake':
				...
				case 'dono':
					...
					teks = `*Nick* : Hiro ModZ\n*Número* : wa.me/557996004602\n*Canal* : youtube.be/channel/UCYM600wnnqMEag26u1CGLVQ`
					buffer = await getBuffer(`https://i.ibb.co/bbf5VmK/51244952d94bbaa6fd58071661e2d0f7.jpg`)
				...
				case 'clearall':
					...
					reply('Sucesso deletar todo o chat :)')
					break
				case 'tm':
					if (!isOwner) return reply('Quem é você?')
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`〔❗〕〔Falha, no momento da conversão ${type} para figurinha〕`)
							})
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('BOT', authorname)} ${ran} -o ${ran}`, async (error) => {
...
break
				...
				case 'info':
					me = client.user
					uptime = process.uptime()
					teks = `*Nome do bot* : DARLING BOT\n*Número do bot* : wa.me/557996004602\n*Prefix* : ${prefix}\n*Bloqueados* : ${blocked.length}\n\n *Para saber do meu criador use o comando .dono*`
					buffer = await getBuffer(`https://i.ibb.co/MMTvjvc/foto.png`)
					client.sendMessage(from, buffer, image, {caption: teks, contextInfo:{mentionedJid: [me.jid]}})
					break
				case 'blocklist':
				...
				case 'add':
					...
					try {
						...
						console.log('Error :', e)
						reply('〔❗〕〔Falha ao adicionar o alvo, talvez porque esteja privado〕')
					}
					break
				case 'kick':
					if (!isGroup) return reply(mess.only.group)
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('〔🆗〕〔Ativado com sucesso o recurso Boas vindas do grupo〕')
					} else if (Number(args[0]) === 0) {
